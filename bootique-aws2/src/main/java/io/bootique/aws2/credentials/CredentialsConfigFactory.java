...
public class CredentialsConfigFactory extends AwsConfigFactory {
    ...
    protected AwsCredentialsProvider createCredentialsProvider(Injector injector) {
        ...
        Set<OrderedCredentialsProvider> diCredentialsProviders = injector.getInstance(Key.getSetOf(OrderedCredentialsProvider.class));
        AwsCredentialsProvider provider2 = createCredentialsProviderChain(diCredentialsProviders);
        if (provider2 != null) {
            return provider2;
        }

        throw new IllegalStateException(
                "No accessKey/secretKey configured, and no alternative credentials providers specified");
    }
